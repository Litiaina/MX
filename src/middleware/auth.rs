...
pub const ACCESS_DGS_MANAGER: i64 = 1;
pub const ACCESS_DGS_ENCODER: i64 = 2;
...
pub fn access_level_name(
    access_level: i64,
) -> &'static str {
    match access_level {
        ACCESS_ADMINISTRATOR => "Administrator",
        ACCESS_DGS_MANAGER => "DGS Manager",
        ACCESS_DGS_ENCODER => "DGS Encoder",
...
impl Claims {
    ...
    pub fn can_read_dgs(&self) -> bool {
        valid_access_level(
            self.access_level
        )
    }

    pub fn can_write_dgs(&self) -> bool {
        matches!(
            self.access_level,
            ACCESS_ADMINISTRATOR
                | ACCESS_DGS_MANAGER
    ...
    pub fn can_delete_dgs(&self) -> bool {
        matches!(
            self.access_level,
            ACCESS_ADMINISTRATOR
                | ACCESS_DGS_MANAGER
...
static VALID_AUTH_KEYS:
    LazyLock<HashSet<String>> =
    LazyLock::new(|| {
        let raw =
            env::var("AUTH_KEYS")
                .unwrap_or_else(
                    |error| {
                        crate::fatal_error!(
                            format!(
                                "failed to load environment variable 'AUTH_KEYS': {}",
                                error
                            ),
                            "static",
                            "VALID_AUTH_KEYS"
...
> {
    ...
    match database_result {
        ...
        Err(error) => {
            crate::report_error!(
                format!(
                    "bootstrap status blocking task failed: {}",
                    error
                ),
                "function",
                "bootstrap_status()"
...
> {
    ...
    match database_result {
        ...
        Err(error) => {
            crate::report_error!(
                format!(
                    "authorization blocking task failed: {}",
                    error
                ),
                "function",
                "authorize()"
...
> {
    ...
    match database_result {
        ...
        Ok(Err(error)) => {
            crate::report_error!(
                format!(
                    "failed to reload user during token refresh: {}",
                    error
                ),
                "function",
                "refresh_access_token()"
        ...
        Err(error) => {
            crate::report_error!(
                format!(
                    "refresh-token database task failed: {}",
                    error
                ),
                "function",
                "refresh_access_token()"
...
> {
    ...
    match database_result {
        ...
        Err(error) => {
            crate::report_error!(
                format!(
                    "session lookup blocking task failed: {}",
                    error
                ),
                "function",
                "session_info()"
        ...
        SystemTime::now()
            ...
            .map_err(
                |error| {
                    crate::report_error!(
                        format!(
                            "failed to calculate JWT expiration: {}",
                            error
                        ),
                        "function",
                        "create_token()"
