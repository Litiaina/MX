...
/*
MX/N1 attachment boundary

SQLite owns record structure, dynamic values, attachment metadata, and revisions.
N1 owns attachment bytes. Every upload targets an Administrator-defined
File Attachment field from Record Structure. The resolved namespace is:

    records/<configured base folders>/<File Attachment field>/<file-name>

If no base folders are configured, storage.rs uses the stable record UID as
the base directory. MX has no built-in business attachment category.
*/
...
impl From<SqliteError> for MxOperationError {
...
fn map_mx_error(error: MxOperationError) -> Response {
    match error {
        MxOperationError::Sqlite(SqliteError::Conflict) => api_json(
            StatusCode::CONFLICT,
            json!({
                "response": "MX entry or attachment conflicts with an existing record."
            }),
        ),

        MxOperationError::Sqlite(SqliteError::NotFound) => api_json(
            StatusCode::NOT_FOUND,
            json!({
                "response": "MX entry or attachment was not found."
            }),
        ),

        MxOperationError::InvalidRequest(reason) => {
            api_json(StatusCode::BAD_REQUEST, json!({ "response": reason }))
        }

        MxOperationError::N1(reason) => {
            crate::report_error!(reason.clone(), "n1", "MX attachment operation");

            api_json(
                StatusCode::BAD_GATEWAY,
                json!({
                    "response":
                        "The MX database is available, but N1 could not complete the attachment operation."
                }),
            )
        }

        MxOperationError::Sqlite(_) => api_json(
...
#[allow(dead_code)]
fn parse_entry_year(date: &str) -> Result<i32, MxOperationError> {
    ...
    let year = date[0..4]
        .parse::<i32>()
        .map_err(|_| MxOperationError::InvalidRequest("Invalid MX entry year.".to_string()))?;

    if !(1900..=9999).contains(&year) {
        return Err(MxOperationError::InvalidRequest(
            "Invalid MX entry year.".to_string(),
        ));
    }
...
fn n1_base_url() -> Result<String, MxOperationError> {
    let value = CONFIG.n1.base_url.trim();

    if value.is_empty() {
        return Err(MxOperationError::N1(
...
fn n1_fragment() -> Result<String, MxOperationError> {
...
fn n1_secret() -> Result<String, MxOperationError> {
    env::var("N1_MX_SECRET")
        .map(|value| value.trim().to_string())
        .map_err(|_| {
            MxOperationError::N1(
                "N1_MX_SECRET is missing from the MX environment/.env.".to_string(),
            )
        })
        .and_then(|value| {
            if value.is_empty() {
                Err(MxOperationError::N1("N1_MX_SECRET is empty.".to_string()))
            } else {
                Ok(value)
            }
        })
...
fn n1_client() -> Result<&'static Client, MxOperationError> {
    ...
    N1_CLIENT
        .get()
        .ok_or_else(|| MxOperationError::N1("failed to initialize N1 HTTP client".to_string()))
}
...
pub(crate) async fn n1_access_token() -> Result<String, MxOperationError> {
    ...
    let response = n1_client()?
        ...
        .send()
        .await
        .map_err(|error| MxOperationError::N1(format!("failed to authenticate to N1: {error}")))?;
    ...
    let auth: N1AuthResponse = serde_json::from_str(&text).map_err(|error| {
        MxOperationError::N1(format!("invalid N1 authentication response: {error}"))
    })?;
...
fn configured_attachment_object_key(
    storage: &ResolvedRecordStorage,
    directory_path: &str,
    file_name: &str,
) -> String {
...
fn mx_preview_cache_path(attachment: &FileAttachment) -> PathBuf {
    let cache_root = env::temp_dir().join("mx-preview-cache");
...
fn generate_office_pdf_preview(
    ...
    let cache_path = mx_preview_cache_path(&attachment);

    if cache_path.is_file() {
        return fs::read(&cache_path).map_err(|error| {
            format!(
                "failed to read cached MX preview '{}': {error}",
                cache_path.display()
            )
        });
    }
    ...
    let work_dir = env::temp_dir().join(format!("mx-preview-work-{}", Uuid::new_v4()));
    ...
    let result = (|| -> Result<Vec<u8>, String> {
        fs::create_dir_all(&profile_dir)
            .map_err(|error| format!("failed to create MX preview work directory: {error}"))?;
        ...
        if let Some(parent) = cache_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|error| format!("failed to create MX preview cache: {error}"))?;
        }

        fs::write(&cache_path, &pdf_bytes)
            .map_err(|error| format!("failed to cache generated MX preview: {error}"))?;
...
/* -------------------------------------------------------------------------- */
/* SQLite: MX record schema                                                 */
/* -------------------------------------------------------------------------- */

pub(crate) fn ensure_mx_record_schema(
    connection: &rusqlite::Connection,
) -> Result<(), rusqlite::Error> {
    crate::api::mx::migration::migrate_legacy_schema(connection)?;
    connection.execute_batch(
        r#"
        CREATE TABLE IF NOT EXISTS mx_control_sequence (
        ...
        CREATE TABLE IF NOT EXISTS mx_attachments (
            uid                           TEXT PRIMARY KEY NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_mx_records_date
            ON mx_records(date DESC, control_no DESC);

        CREATE INDEX IF NOT EXISTS idx_mx_records_office
            ON mx_records(office COLLATE NOCASE);

        CREATE INDEX IF NOT EXISTS idx_mx_records_requestor
            ON mx_records(requestor COLLATE NOCASE);
        ...
        CREATE INDEX IF NOT EXISTS idx_mx_records_office_date
            ON mx_records(office COLLATE NOCASE, date DESC, control_no DESC);

        CREATE INDEX IF NOT EXISTS idx_mx_records_route_date
            ON mx_records(routed_to_div COLLATE NOCASE, date DESC, control_no DESC);

        CREATE INDEX IF NOT EXISTS idx_mx_records_requestor_date
            ON mx_records(requestor COLLATE NOCASE, date DESC, control_no DESC);
        ...
        INSERT OR IGNORE INTO mx_revision (
            id,
            records_revision
        ) VALUES (
            1,
        ...
        CREATE TRIGGER IF NOT EXISTS trg_mx_records_revision_insert
        AFTER INSERT ON mx_records
        BEGIN
            UPDATE mx_revision
        ...
        CREATE TRIGGER IF NOT EXISTS trg_mx_records_revision_update
        AFTER UPDATE ON mx_records
        BEGIN
            UPDATE mx_revision
            SET records_revision = records_revision + 1
            WHERE id = 1;
        ...
        CREATE TRIGGER IF NOT EXISTS trg_mx_records_revision_delete
        AFTER DELETE ON mx_records
        BEGIN
            UPDATE mx_revision
            SET records_revision = records_revision + 1
            WHERE id = 1;
        ...
        CREATE TRIGGER IF NOT EXISTS trg_mx_attachments_revision_insert
        AFTER INSERT ON mx_attachments
        BEGIN
            UPDATE mx_revision
            SET records_revision = records_revision + 1
            WHERE id = 1;
        ...
        CREATE TRIGGER IF NOT EXISTS trg_mx_attachments_revision_update
        AFTER UPDATE ON mx_attachments
        BEGIN
            UPDATE mx_revision
            SET records_revision = records_revision + 1
            WHERE id = 1;
        ...
        CREATE TRIGGER IF NOT EXISTS trg_mx_attachments_revision_delete
        AFTER DELETE ON mx_attachments
        BEGIN
            UPDATE mx_revision
            SET records_revision = records_revision + 1
            WHERE id = 1;
...
#[allow(dead_code)]
fn build_mx_where_clause(query: &MxListQuery) -> (String, Vec<SqlValue>) {
...
#[allow(dead_code)]
fn build_order_clause(query: &MxListQuery) -> String {
...
#[allow(dead_code)]
pub async fn execute_create_mx_record(
    ...
    let database_result =
        tokio::task::spawn_blocking(move || -> Result<Entry, SqliteDatabaseError> {
            with_sql_connection(|connection| {
                ...
                /*
                The sequence increment and the new MX row live in the same
                SQLite transaction.
...
#[allow(dead_code)]
async fn execute_update_mx_record_db(
    uid: String,
...
#[allow(dead_code)]
pub async fn execute_list_mx_records(
    query: MxListQuery,
    content_type: &str,
    function_name: &str,
) -> Result<MxPage, SqliteError> {
    ...
    let database_result =
        tokio::task::spawn_blocking(move || -> Result<MxPage, SqliteDatabaseError> {
            with_sql_connection(|connection| {
                ensure_mx_record_schema(connection)?;
                ensure_attachment_fields_schema(connection)?;
                let (where_clause, base_params) = build_mx_where_clause(&query);
                ...
                let exists = connection
                    .query_row(
                        r#"
                            ...
                            FROM mx_attachments
                            WHERE entry_uid = ?1
                              AND attachment_field_uid = ?2
                              AND file_name = ?3 COLLATE NOCASE
                            LIMIT 1
                            "#,
                        params![entry_uid, attachment_field_uid, file_name,],
                        |_row| Ok(true),
                    )
                    .optional()?
                    .unwrap_or(false);
...
async fn execute_attachment_object_key_exists(object_key: String) -> Result<bool, SqliteError> {
    let result = tokio::task::spawn_blocking(move || -> Result<bool, SqliteDatabaseError> {
        with_sql_connection(|connection| {
            ensure_mx_record_schema(connection)?;
            ensure_attachment_fields_schema(connection)?;
            Ok(connection.query_row(
                "SELECT EXISTS(SELECT 1 FROM mx_attachments WHERE object_key=?1)",
                params![object_key],
                |row| row.get::<_, i64>(0),
            )? != 0)
        })
            ...
            with_sql_connection(|connection| {
                ...
                connection.execute(
                    ...
                    params![
                        ...
                        attachment.attachment_field_uid,
                        attachment.attachment_field_label.unwrap_or_default(),
                        attachment.attachment_field_storage_name.unwrap_or_default(),
                    ],
                )?;
...
pub async fn execute_upload_mx_attachment(
    entry_uid: String,
    attachment_field_uid: String,
    file_name: String,
    mime_type: String,
    ...
    if bytes.len() > max_size {
        return Err(MxOperationError::InvalidRequest(format!(
            "Attachment exceeds the configured {} MiB MX limit.",
            CONFIG.n1.attachment_max_size_mb.max(1)
        )));
    }

    // Filenames are unique inside one File Attachment field. Two different
    // attachment fields may intentionally contain the same filename.
    // This avoids silent N1 namespace collisions and remains backward compatible.
    if execute_attachment_name_exists(
        entry_uid.clone(),
        attachment_field_uid.clone(),
        file_name.clone(),
    )
    .await?
    {
        return Err(MxOperationError::InvalidRequest(format!(
            "The record already has an attachment named '{file_name}'. Rename the new file or remove the old attachment first."
        )));
    }

    let field_uid = attachment_field_uid.trim().to_string();
    if field_uid.is_empty() {
        return Err(MxOperationError::InvalidRequest(
            "A File Attachment field is required for every upload.".to_string(),
        ));
    }

    let field_uid_for_lookup = field_uid.clone();
    ...
    let field = match result {
        ...
        Ok(Ok(None)) => {
            return Err(MxOperationError::InvalidRequest(
                "The selected File Attachment field does not exist or is archived.".to_string(),
            ));
        }
        _ => {
            return Err(MxOperationError::Sqlite(SqliteError::SqliteDatabaseError));
        }
    };

    // Enforce single/multiple/max-files at the server, not only in the UI.
    let field_uid_for_count = field.uid.clone();
    let entry_for_count = entry_uid.clone();
    let count_result = tokio::task::spawn_blocking(
        move || -> Result<u64, SqliteDatabaseError> {
            with_sql_connection(|connection| {
                ensure_attachment_fields_schema(connection)?;
                let count: i64 = connection.query_row(
                    "SELECT COUNT(*) FROM mx_attachments WHERE entry_uid=?1 AND attachment_field_uid=?2",
                    params![entry_for_count, field_uid_for_count],
                    |row| row.get(0),
                )?;
    ...
    .await;
    let current_count = match count_result {
        Ok(Ok(value)) => value,
        _ => {
            return Err(MxOperationError::Sqlite(SqliteError::SqliteDatabaseError));
        }
    ...
    if !field.multiple && current_count >= 1 {
        return Err(MxOperationError::InvalidRequest(format!(
    ...
    let storage = match resolve_record_storage(entry_uid.clone()).await {
        ...
        Err(StorageResolutionError::NotFound) => {
            return Err(MxOperationError::Sqlite(SqliteError::NotFound));
        }
    ...
    // File Attachment fields become the final directory segment:
    // records/<chosen field>/<optional chosen field>/<attachment field>/file.pdf
    let directory_path = format!(
        "{}/{}",
        storage.directory_path.trim_end_matches('/'),
        sanitize_namespace_component(&field.storage_name, "Attachments")
    );

    let object_key = configured_attachment_object_key(&storage, &directory_path, &file_name);
    if execute_attachment_object_key_exists(object_key.clone()).await? {
        return Err(MxOperationError::InvalidRequest(format!(
            "N1 storage path collision for '{object_key}'. Add a unique filename prefix field, another folder field, or rename the file."
        )));
    }
    ...
    let attachment = FileAttachment {
        ...
        version_id: None,
        attachment_field_uid: Some(field.uid.clone()),
        attachment_field_label: Some(field.label.clone()),
...
async fn execute_update_entry_with_n1_moves(
    ...
    operation instead of silently renumbering an existing MX record.
    */
    if new_year != existing.control_year {
        return Err(MxOperationError::InvalidRequest(format!(
            "The entry year cannot be changed from {} to {} after control number {} has been assigned. Create a new MX entry instead.",
...
#[allow(dead_code)]
pub async fn create_mx_record(claims: Claims, Json(request): Json<CreateEntryRequest>) -> Response {
    ...
    match execute_create_mx_record(request, year, "api", "create_mx_record()").await {
        Ok(entry) => api_json(StatusCode::CREATED, json!(entry)),

        Err(error) => map_mx_error(MxOperationError::Sqlite(error)),
    }
}
...
#[allow(dead_code)]
pub async fn list_mx_records(claims: Claims, Query(query): Query<MxListQuery>) -> Response {
    if !claims.can_read_records() {
        return mx_access_denied();
    }

    match execute_list_mx_records(query, "api", "list_mx_records()").await {
...
async fn upload_mx_attachments_inner(
    ...
    loop {
        let field = match multipart.next_field().await {
            ...
            Err(error) => {
                return api_json(
                    ...
                    json!({"response": format!("Invalid multipart upload: {error}")}),
                );
            }
        };
        ...
        let bytes = match field.bytes().await {
            ...
            Err(error) => {
                return api_json(
                    StatusCode::BAD_REQUEST,
                    json!({"response": format!("Failed to read attachment '{file_name}': {error}")}),
                );
            }
        };

        match execute_upload_mx_attachment(
            entry_uid.clone(),
            attachment_field_uid.clone(),
            file_name,
            mime_type,
    ...
    if uploaded.is_empty() {
        return api_json(
            StatusCode::BAD_REQUEST,
            json!({"response":"No files were provided. Multipart field name must be 'files'."}),
        );
    }
...
pub async fn upload_mx_attachment_field(
    claims: Claims,
    Path((entry_uid, attachment_field_uid)): Path<(String, String)>,
    multipart: Multipart,
) -> Response {
    if !claims.can_write_records() {
        return mx_access_denied();
...
pub async fn preview_mx_attachment(
    ...
    if office_preview_supported(&attachment.file_name) {
        ...
        let pdf_bytes = match preview_result {
            ...
            Ok(Err(error)) => {
                crate::report_error!(error, "preview", "preview_mx_attachment()");

                return api_json(
                    StatusCode::SERVICE_UNAVAILABLE,
                    json!({
                        "response": "MX could not generate the Office preview. Verify that LibreOffice Writer, Calc, and Impress are installed on the MX server."
                    }),
                );
            }
            ...
            Err(error) => {
                crate::report_error!(
                    ...
                    "preview",
                    "preview_mx_attachment()"
                );

                return api_json(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    json!({
                        "response": "MX Office preview task failed."
                    }),
                );
            }
        };
