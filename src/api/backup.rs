...
fn backup_error(status: StatusCode, message: impl Into<String>) -> Response {
    (
        status,
        Json(json!({
...
async fn create_sqlite_snapshot(destination: PathBuf) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), SqliteDatabaseError> {
        with_sql_connection(|connection| {
            ...
            let sql = format!("VACUUM INTO '{}';", sqlite_literal(&destination),);
...
fn verify_sqlite_file(path: &FsPath) -> Result<String, String> {
    ...
    let mut statement = connection
        ...
        .map_err(|error| format!("failed to prepare backup integrity check: {error}"))?;

    let mut rows = statement
        .query([])
        .map_err(|error| format!("failed to execute backup integrity check: {error}"))?;

    let mut messages = Vec::new();

    while let Some(row) = rows
        .next()
        .map_err(|error| format!("failed to read backup integrity result: {error}"))?
    {
        messages.push(
            row.get::<_, String>(0)
                .map_err(|error| format!("failed to decode backup integrity result: {error}"))?,
        );
    }
    ...
    if result.eq_ignore_ascii_case("ok") {
        ...
        Err(if result.is_empty() {
            "SQLite integrity_check returned no result.".to_string()
        } else {
...
async fn insert_backup_entry(entry: BackupEntry) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), SqliteDatabaseError> {
        with_sql_connection(|connection| {
            ensure_backup_schema(connection)?;
...
async fn get_backup_entry(uid: String) -> Result<Option<BackupEntry>, String> {
    tokio::task::spawn_blocking(
        move || -> Result<Option<BackupEntry>, SqliteDatabaseError> {
            with_sql_connection(|connection| {
                ensure_backup_schema(connection)?;
        ...
        with_sql_connection(|connection| {
            ...
            connection.execute(
                r#"
                    ...
                    WHERE uid = ?4
                    "#,
                params![status, integrity_check, verified_at, uid,],
            )?;
...
pub async fn list_backups(claims: Claims) -> Response {
    ...
    let result =
        tokio::task::spawn_blocking(move || -> Result<Vec<BackupEntry>, SqliteDatabaseError> {
            with_sql_connection(|connection| {
                ensure_backup_schema(connection)?;
                ...
                let rows = statement.query_map([], backup_row)?;
    ...
    match result {
        ...
        Ok(Err(error)) => {
            crate::report_error!(format!("{error}"), "backup", "list_backups()");
        ...
        Err(error) => {
            crate::report_error!(
                format!("backup list blocking task failed: {error}"),
                "backup",
                "list_backups()"
            );
...
pub async fn create_backup(claims: Claims) -> Response {
    if !claims.can_manage_accounts() {
        return backup_access_denied();
    }

    let _guard = match BackupRunGuard::acquire() {
        Ok(guard) => guard,
    ...
    let temp_directory = match backup_temp_directory() {
        ...
        Err(error) => {
            return backup_error(StatusCode::INTERNAL_SERVER_ERROR, error);
        }
    };

    let temp_path = temp_directory.join(format!("{}-{}", uid, file_name));

    if let Err(error) = create_sqlite_snapshot(temp_path.clone()).await {
    ...
    let integrity_check = match verify_sqlite_file_async(temp_path.clone()).await {
        Ok(result) => result,
    ...
    let bytes = match read_backup_bytes(temp_path.clone()).await {
        Ok(bytes) => bytes,
...
pub async fn verify_backup(claims: Claims, Path(uid): Path<String>) -> Response {
    if !claims.can_manage_accounts() {
        return backup_access_denied();
    }

    let backup = match get_backup_entry(uid.clone()).await {
        ...
        Err(error) => {
            crate::report_error!(error.clone(), "backup", "verify_backup()");

            return backup_error(StatusCode::INTERNAL_SERVER_ERROR, error);
        }
    };

    let bytes = match n1_download(&backup.object_key, &backup.file_name).await {
        Ok(bytes) => bytes,

        Err(error) => {
            crate::report_error!(format!("{error:?}"), "backup", "verify_backup()");
    ...
    if bytes.len() as u64 != backup.size {
        let message = format!(
        ...
        let _ = update_backup_verification(
            uid,
            "failed".to_string(),
            message.clone(),
        ...
        return backup_error(StatusCode::CONFLICT, message);
    }
    ...
    let temp_path = temp_directory.join(format!("verify-{}-{}", backup.uid, backup.file_name));
    ...
    let verification = verify_sqlite_file_async(temp_path.clone()).await;
    ...
    match verification {
        Ok(integrity_check) => {
            if let Err(error) = update_backup_verification(
                backup.uid.clone(),
                "verified".to_string(),
        ...
        Err(error) => {
            ...
            backup_error(StatusCode::CONFLICT, error)
        }
    }
}

pub async fn download_backup(claims: Claims, Path(uid): Path<String>) -> Response {
    if !claims.can_manage_accounts() {
        return backup_access_denied();
    }

    let backup = match get_backup_entry(uid).await {
        Ok(Some(backup)) => backup,
    ...
    let bytes = match n1_download(&backup.object_key, &backup.file_name).await {
        ...
        Err(error) => {
            crate::report_error!(format!("{error:?}"), "backup", "download_backup()");
    ...
    let safe_name = backup.file_name.replace('"', "_");
